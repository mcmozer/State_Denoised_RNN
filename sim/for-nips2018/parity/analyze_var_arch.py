...
for arch_type in ['tanh','GRU']:

    seq_len = [10]
    condition = ['sdprnn_15steps_noise500','rnn','rnna_15steps_noise500'];
    ...
    for lix in range(len(seq_len)):
        for cix in range(len(condition)):
            ...
            file = open('results/'+condition[cix]+'_'+arch_type+'.out','r')
    ...
    for cix in range(len(condition)):
        print('%-35s ' % (condition[cix] + '_' + arch_type)),
        for lix in range(len(seq_len)):
