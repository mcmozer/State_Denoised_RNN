...
def attractor_net_params_init():
    ...
    W = {'hid': tf.get_variable("W_hid",initializer=.01*tf.random_normal([N_HIDDEN,N_HIDDEN])),
         'in':  tf.get_variable("W_in", initializer=tf.eye(N_INPUT,num_columns=N_HIDDEN)+.01*tf.random_normal([N_INPUT,N_HIDDEN])),
         'out': tf.get_variable("W_out",initializer=tf.eye(N_HIDDEN,num_columns=N_INPUT)+.01*tf.random_normal([N_HIDDEN,N_INPUT])),
        }
...
def attractor_net(X, params, Y):
    ...
    hplus = b['in'] + tf.matmul(tf.atanh(X), W['in'])
    ...
    for i in range(1,N_ATTRACTOR_STEPS+1):
        h = tf.concat([h, tf.expand_dims(
                              #tf.matmul(h[i-1,:,:], W_hid_constr) + hplus,0)], 0)
    ...
    h_final = tf.tanh(b['out'] + tf.matmul(h[N_ATTRACTOR_STEPS,:,:], W['out']))
...
Y_, W_hid_constr, loss_op = attractor_net(X, params, Y)
