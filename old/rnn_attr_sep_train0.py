...
SEQ_LEN = 10          # number of bits in input sequence   DEBUG *********************
...
ATTRACTOR_WEIGHT_CONSTRAINTS = True
...
def attractor_net_loss_function(attractor_tgt_net, params):
    ...
    attr_loss = tf.reduce_mean(tf.pow(attr_tgt - tf.tanh(a_cleaned),2)) / \
...
attractor_weights = params['W']['attractor']
attractor_bias = params['b']['attractor']
params_without_attractor_weights = [params['W'][key] for key in params['W']] + \
                                   [params['b'][key] for key in params['b']]
if (not TRAIN_ATTRACTOR_WEIGHTS_ON_PREDICTION_TOO):
    params_without_attractor_weights.remove(attractor_weights)
    params_without_attractor_weights.remove(attractor_bias)
...
pred_train_op = optimizer_pred.minimize(pred_loss_op, 
                                   var_list=params_without_attractor_weights)
# Define optimizer for attractor net task
optimizer_attr = tf.train.AdamOptimizer(learning_rate=LRATE_ATTRACTOR)
attr_train_op = optimizer_attr.minimize(attr_loss_op, var_list=[attractor_weights, 
                                                                attractor_bias])
...
with tf.Session() as sess:
    ...
    for replication in range(n_replications):
        ...
        print (attractor_weights.eval())
        print (attractor_bias.eval())
        # test performance
        if (0):
            accs = []
