...
ATTRACTOR_UPDATE_FREQ = 1 # how often (in epochs) to update attractor weights
...
NOISE_LEVEL = 0.25    # noise in training attractor net
N_ATTRACTOR_STEPS = 5 # number of time steps in attractor dynamics   # DEBUG*****
ATTRACTOR_WEIGHT_CONSTRAINTS = False #************** DEBUG
                      # True: make attractor weights symmetric and have zero diag
                      # False: unconstrained
TRAIN_ATTRACTOR_WEIGHTS_ON_PREDICTION_TOO = False
...
def GRU(X, params):
    with tf.variable_scope("GRU"):
        W = params['W']
        b = params['b']
        if ATTRACTOR_WEIGHT_CONSTRAINTS:
...
def RNN_tanh(X, params):
    ...
    def _step(accumulated_vars, input_vars):
        ...
        # clean-up for prediction net
        h_cleaned = tf.zeros(tf.shape(h_prev))
        for _ in range(N_ATTRACTOR_STEPS):
            h_cleaned = tf.matmul(tf.tanh(h_cleaned), W_attractor_constrained) \
                                                              + h_net + b['attractor']
        h_cleaned = tf.tanh(tf.matmul(h_cleaned, W['aout']) + b['aout'])
...
def attractor_net_loss_function(attractor_tgt_net, params):
    ...
    a_cleaned = tf.tanh(tf.matmul(a_cleaned, W['aout']) + b['aout'])
...
attr_weights = params['W']['attractor']
attr_bias = params['b']['attractor']
aout_weights = params['W']['aout']
aout_bias = params['b']['aout']
params_without_attr = [params['W'][key] for key in params['W']] + \
                      [params['b'][key] for key in params['b']]
if (not TRAIN_ATTRACTOR_WEIGHTS_ON_PREDICTION_TOO):
    params_without_attr.remove(attr_weights)
    params_without_attr.remove(attr_bias)
    params_without_attr.remove(aout_weights)
    params_without_attr.remove(aout_bias)
...
pred_train_op = optimizer_pred.minimize(pred_loss_op, 
                                   var_list=params_without_attr)
# Define optimizer for attractor net task
optimizer_attr = tf.train.AdamOptimizer(learning_rate=LRATE_ATTRACTOR)
attr_train_op = optimizer_attr.minimize(attr_loss_op, 
                            var_list=[attr_weights, attr_bias, aout_weights, aout_bias])
    ...
    for replication in range(n_replications):
        ...
        for epoch in range(1, training_epochs + 2):
            ...
            for (batch_x, batch_y) in batches:
                ...
                if (epoch-1) % ATTRACTOR_UPDATE_FREQ == 0:
        ...
        print (attr_weights.eval())
        print (attr_bias.eval())
        # test performance
        if (0):
            accs = []
